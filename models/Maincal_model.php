...
class Maincal_model extends CI_Model{
    ...
    function get_calendar_data($year,$month){
	   ...
	   $query=$this->db->select('date,time,data','public')
				->from('events')
				->like('date',"$year-$month",'after')
			   ->where($array1)->or_where($array2)
			   ->where( 'uid=',$_SESSION['user_id'])
			   ->like('date',"$year-$month",'after')->order_by('time','ASC')
               ->get();
          ...
          foreach($query->result() as $row){
		   ...
		   if(substr($temp_day, 0, 1) == 0) {
			...
			if(isset($cal_data[$temp_day])){
				$cal_data[$temp_day]=$cal_data[$temp_day].
				"<div class='grammi'>".
				$urow->username."|".$row->time."|".
				$row->data."</div>";
			}
			else{
				$cal_data[$temp_day]="<div class='grammi'>".
				$urow->username."|".$row->time."|".
				$row->data."</div>";
